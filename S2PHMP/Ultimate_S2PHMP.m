...
%         minEp = 0;
        ...
        for i = 1:Num_SingleTest
            [A1, A2] = RandMatrixGen2(N,minEp, maxEp, FirstNumMin, FirstNumMax);
            [B1, B2] = RandMatrixGen2(N,minEp, maxEp, FirstNumMin, FirstNumMax);
            
            [Va, Vb, A1_result, A2_result, B1_result, B2_result, Re_Err_result, Ab_Err_result, MRE_result, MAPE_result, SAPE_result, F_norm,...
                Theory_result, Real_result, Error_Rate] = S2PHMP(A1, A2, B1, B2);
            ...
            A1_copy = cat(3,A1_copy,A1_result);
            A2_copy = cat(3,A1_copy,A1_result);
            
            B1_copy = cat(3,B1_copy,B1_result);
            B2_copy = cat(3,B1_copy,B1_result);
        ...
        A1 = A1_copy(:, :, index_TestNum);
        A2 = A2_copy(:, :, index_TestNum);
        B1 = B1_copy(:, :, index_TestNum);
        B2 = B2_copy(:, :, index_TestNum);
...
filename = strcat('【Ep=(', num2str(minEp),',',num2str(maxEp),')Dim=(',...
    num2str(DimMin),',',num2str(DimMax),')】S2PHMP_ExperimentResults.xlsx');
